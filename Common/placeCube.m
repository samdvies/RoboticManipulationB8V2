function placeCube(hw, target_xy, stack_level, is_rotated, pickup_angle_deg, cfg, release_z_adjust, cube_index)
...
%   pickup_angle_deg - PICK angle (used for angle-based offset when cube_index not used).
%   cfg              - config struct from task_config()
%   release_z_adjust - (optional) mm to add to release height; negative = lower (default 0)
%   cube_index       - (optional) when provided and cfg.cube_place_offsets exists, use
%                      per-cube [radial, tangential] for this cube instead of angle-based offset.

if nargin < 7, release_z_adjust = 0; end
if nargin < 8, cube_index = []; end
...
% Per-cube offset (Task 2a) or angle-based offset
use_per_cube = ~isempty(cube_index) && isfield(cfg, 'cube_place_offsets') && ...
    cube_index >= 1 && cube_index <= size(cfg.cube_place_offsets, 1);

if use_per_cube
    po = cfg.cube_place_offsets(cube_index, :);
    off_x = 0;
    off_y = 0;
    % Debug: confirm per-cube offset is used (radial, tangential in mm)
    fprintf('    [place] Per-cube offset cube_index=%d  [radial=%.1f, tangent=%.1f]\n', cube_index, po(1), po(2));
else
    if is_rotated
        ...
        off_x = 0;
        off_y = 0;
        if isfield(cfg, 'PLACE_OFFSET_STD_0')
            if abs(pickup_angle_deg - 0) < 1
...
% Bearing from base to target: avoid atan2(0,0) when target is on Y-axis (e.g. 0,175)
if tx == 0 && ty == 0
    bearing_place = 0;
elseif tx == 0
    bearing_place = sign(ty) * (pi/2);
else
...
tuning = [0, 0];
if isfield(cfg, 'place_offset_tuning') && numel(cfg.place_offset_tuning) >= 2
    tuning = cfg.place_offset_tuning(1:2);
end
act_x = tx + off_x + po_wx + tuning(1);
act_y = ty + off_y + po_wy + tuning(2);

fprintf('    [place] Target (%.1f, %.1f) stack=%d  pick_angle=%.1f°  offset=(%.2f, %.2f)  actual=(%.1f, %.1f) z=%.1f work_pitch=%.0f transit_pitch=%.0f\n', ...
    tx, ty, stack_level, pickup_angle_deg, off_x + po_wx + tuning(1), off_y + po_wy + tuning(2), act_x, act_y, place_z, work_pitch, transit_pitch);
